					...
					<div class="modal-body text-start">
						...
						<div class="mb-2">
							...
							<select v-model="addForm.shop_id" class="form-select form-select-sm">
								<option value="">—</option>
								<option v-for="shop in shopSelectOptions" :key="shop.name" :value="shop.id">
									{{ shop.name }}
								</option>
							</select>
						</div>
						...
						<div class="mb-2">
							...
							<input v-model="addForm.warranty_months" type="number" min="1" class="form-control form-control-sm"
								required />
						</div>
...
<table class="table table-sm table-striped table-hover table-responsive">
	...
	<tbody>
		<tr v-for="item in items" :key="item.id">
			...
			<td>{{ item.name }}</td>
			<td>
				<button v-if="item.shop_name" type="button" class="btn btn-outline-primary btn-sm" data-bs-toggle="modal"
					:data-bs-target="`#shopView_${item.id}`" @click="openShopDetails(item.shop_id)">
					{{ item.shop_name }}
				</button>
			</td>
...
<pagination-bar :page="page" :pages="pages" @change="loadItems" />
			...
			<div class="modal-content">
				...
				<form @submit.prevent="submitEdit(item.id)">
					<div class="modal-body text-start">
						...
						<div class="mb-2">
							<label class="form-label">Price per piece</label>
							<input v-model="editForms[item.id].price_per_piece" type="number" step="0.01"
						...
						<div class="mb-2">
							<label class="form-label">Purchase date*</label>
							<input v-model="editForms[item.id].purchase_date" type="date" class="form-control form-control-sm"
								required />
						</div>
						<div class="mb-2">
							<label class="form-label">Warranty length (months)*</label>
							<input v-model="editForms[item.id].warranty_months" type="number" min="1"
