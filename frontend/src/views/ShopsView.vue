...
async function submitAdd() {
  try {
    ...
    Object.assign(addForm, { name: '', street: '', city: '', zip_code: '' })
    await loadShops(1)
  } catch (error) {
    showAlert(error.response?.data?.detail || 'Failed to add shop.', 'danger')
  }
}
...
async function removeShop(shopId, linkedItems = false) {
  ...
  showAlert(linkedItems ? 'The record and linked items have been successfully deleted.' : 'The record has been successfully deleted.')
  await loadShops()
}
...
<template>
        ...
        <div class="modal-content">
          ...
          <form @submit.prevent="submitAdd">
            <div class="modal-body text-start">
              ...
              <button type="submit" class="btn btn-sm btn-primary">Add shop</button>
      ...
      <tr v-for="shop in shops" :key="shop.id">
        ...
        <td>
          <button
            ...
            :data-bs-target="`#shopItems_${shop.id}`"
            @click="loadWarrantyItems(shop.id)"
          >
            {{ shop.items_count }}
          </button>
          <span v-else>0</span>
  ...
  <PaginationBar :page="page" :pages="pages" @change="loadShops" />
      ...
      <div class="modal-dialog modal-md">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <WarrantyItemsTable
              v-if="warrantyItems[shop.id]"
              :under-warranty="warrantyItems[shop.id].under_warranty"
